...
// ── Helpers format angka Indonesia ───────────────────────────────────────────
...
export default function FormPinjaman({ onClose, onSuccess }: Props) {
  ...
  // Nominal — simpan sebagai string tampilan (dengan titik), angka sebenarnya di-parse saat dipakai
  ...
  // Handler input manual dengan format titik otomatis
  const handleNominalInput = (e: React.ChangeEvent<HTMLInputElement>) => {
    const raw = e.target.value.replace(/\./g, '') // hapus titik dulu
    if (raw === '') { setNominalDisplay(''); return }
    if (!/^\d+$/.test(raw)) return // tolak non-digit
    setNominalDisplay(toDisplayValue(raw))
  }

  // Handler tombol preset
          ...
          <section>
            ...
            <div className="grid grid-cols-3 gap-3">
              ...
              <div className="col-span-3 space-y-2">
                ...
                {/* Input manual dengan format titik */}
                <div className="relative">
                  <span className="absolute left-3 top-1/2 -translate-y-1/2 text-sm text-ink-400 font-semibold select-none">
                    Rp
                  </span>
                  <input
                    type="text"
                ...
                {/* Preview terbaca */}
                {nominal > 0 && (
                  <p className="text-[11px] text-ink-400 pl-1">
                    = <span className="font-semibold text-ink-600">{formatRupiah(nominal)}</span>
                  </p>
                )}
              </div>
              ...
              <div>
                ...
                <input
                  type="number" min={0} max={100} step={0.5} value={bunga}
              ...
              <div className="col-span-2">
                ...
                <div className="relative">
                  <select
                    value={lama}
                    onChange={(e) => setLama(Number(e.target.value))}
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-1.5">
                          ...
                          {s.min_nominal && (
                            <span className="text-[9px] text-ink-300">≥ {formatRupiah(s.min_nominal)}</span>
                          )}
                        </div>
                        {uploaded && (
                          <p className="text-[10px] text-emerald-600 mt-0.5 truncate">{uploads[s.kode]?.name}</p>
                        )}
                      </div>
                      <div>
                        <input
                        ...
                        />
                        <button
                          onClick={() => fileRefs.current[s.kode]?.click()}
